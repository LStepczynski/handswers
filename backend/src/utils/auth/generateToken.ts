 ...
 *
 * @param {JwtUser} payload - The data to encode in the JWT, typically containing user-specific details (e.g., ID, username).
 *
 * @returns {string} The generated JWT.
 */
export const generateToken = (payload: jwt.JwtPayload) => {
  const secret = process.env.JWT_ACCESS_SECRET || "default";
  const expireAfter = Number(process.env.JWT_ACCESS_EXPIRATION) || 0.5;
 ...
 * @returns {string} The generated JWT.
 */
export const generateRefresh = (payload: JwtUser) => {
